...
def make_captions(image_dir, trigger_word):
    for filename in os.listdir(image_dir):
        if filename.lower().endswith('_cropped.png'):
            ...
            text_file_path = os.path.join(image_dir, text_filename)
            
            # Write the text "hello" to the text file
            with open(text_file_path, 'w') as file:
                file.write(trigger_word)
...
def make_archive(archive_path, files):
...
def main():
    ...
    cl_args = parse_experiment_settings()
    print(cl_args)


    # Directory containing the images
    image_dir = cl_args['dir']
    ...
    for filename in os.listdir(image_dir):
        ...
        if filename.lower().endswith(('png', 'jpg', 'jpeg', 'bmp', 'gif')):
            ...
            # 1. Crop to 1024x1024
            cropped_img = resize_and_center_crop(img)
    ...
    make_captions(image_dir, trigger_word=cl_args['trigger'])

    # 4. Create zip file with all images and their captions.
    make_archive(cl_args['zip_to'], cropped_images)
